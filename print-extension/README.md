...
5. **系统打印机（USB）打印实现方式**
   - 本服务通过 **Windows 计划任务** 在用户会话中执行打印脚本（手动 CMD 能出纸、Node 子进程不能时，计划任务与手动环境一致）。首次使用系统打印机打印时，若任务不存在会自动创建任务名 **ZPLPrintFromNode**（无需管理员），之后每次打印会触发该任务执行。
   - 若仍无队列、未出纸：可在「任务计划程序」中查看 **ZPLPrintFromNode** 是否被禁用，或改用 **TCP**（打印机 IP:9100）连接打印。
