...
5. **Zebra USB（如 ZD621R300dpi）：队列无任务、未出纸**
   - 本服务已通过 Windows 驱动提交 RAW+Passthrough（`${ ... }$`），但部分 Zebra USB 驱动不会在队列中显示任务且不出纸，属驱动/系统行为。
   - **推荐做法**：若打印机带网口，在连接打印页用 **TCP 方式** 添加（打印机 IP、端口 9100），选中该 TCP 打印机打印，可稳定出纸。
   - 可选尝试：打印机右键 → 打印首选项 → 高级/其他 开启 **Passthrough 模式**（起止 `${` / `}$`）；或 打印机属性 → 高级 中切换「直接打印到打印机」与「使用后台打印」。
