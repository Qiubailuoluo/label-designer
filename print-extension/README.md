...
## 部署与发布说明（前端上公网 + 扩展上架 + 用户本机）

| 组成部分 | 部署位置 | 说明 |
|----------|----------|------|
| **前端 / 后端业务** | 公网服务器 | 你的 Vue 前端、API 等照常部署到公网，用户通过浏览器访问。 |
| **Chrome 扩展** | **扩展商店**（如 Chrome 网上应用店） | 只发布 **`print-extension/extension`** 目录（manifest、background.js、content.js 等）。扩展里写死或配置请求 `http://127.0.0.1:8765`，用于和用户本机的打印服务通信。**不要**把 `local-print-service`（Node 服务）打包进扩展，商店也不运行 Node。 |
| **本地打印服务** | **每台需要打印的用户电脑本机** | 需要打印的用户，必须在自己电脑上运行 **`node server.js`**（在 `local-print-service` 目录下）。因为打印机接在这台电脑上，只有本机进程才能访问系统打印机、发 TCP 到局域网打印机。不能把这段服务部署到公网，否则无法操作用户本地打印机。 |

**总结**：  
- **扩展** → 上架到扩展商店，用户从商店安装；扩展只负责「页面 ↔ 本机 8765」的桥接。  
- **本地打印服务** → 不上架、不放公网；需要打印的用户自行在本机运行（或你提供安装包/一键启动脚本）。你可以在产品里提供 `local-print-service` 的下载与使用说明（例如从 GitHub Releases 或你的官网下载 zip，解压后运行 `node server.js` 或你打包的 exe）。
...
3. **在「连接打印」页测试**
   ...
   - **方式三：USB 打印机（应用连接）**  
     连接方式选 USB，设备/端口填 Windows 端口名（如 **COM3**、**USB001**，可在「设备和打印机」中查看该打印机属性→端口），点「应用连接」。选中该 USB 打印机后打印会直写该端口发送 ZPL。  
     **若显示发送成功但不出纸**：多为该端口被系统打印机驱动占用（如 ZD621R300dpi 已使用 USB001），数据被驱动接管。可改为在左侧选**系统打印机「ZD621R300dpi」**再打印，或若打印机带网口则用 TCP 方式添加后打印。

4. 若提示「刷新打印机列表失败」或「打印失败」，请确认：本地打印服务已启动、打印机已开机且未脱机、Zebra 驱动已安装。

5. **系统打印机（USB）打印：需先运行一次安装脚本**
   - 使用系统打印机（如 ZD621R300dpi）前，请在本机 **运行一次** `local-print-service` 目录下的 **`setup-print-task.bat`**，用于创建计划任务 **ZPLPrintFromNode**（仅需一次）。之后启动 `node server.js` 并选系统打印机即可出纸。
   - 若未运行过该脚本就选系统打印机，会提示「未找到打印计划任务」，按提示运行 `setup-print-task.bat` 即可。
   - 部分电脑在运行该脚本或首次打印时可能弹出安全/病毒提示，属计划任务触发的正常行为，请选择 **允许** 或 **仍要运行**。
   - 若仍无队列、未出纸：在「任务计划程序」中查看 **ZPLPrintFromNode** 是否被禁用，或改用 **TCP**（打印机 IP:9100）连接打印。
