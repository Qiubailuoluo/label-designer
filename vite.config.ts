...
function getWindowsPrinterList(): { id: string; name: string; address: string }[] {
  ...
  try {
    ...
    const data = JSON.parse(out.trim() || '[]') as { Name?: string; name?: string; PortName?: string; portName?: string } | { Name?: string; name?: string; PortName?: string; portName?: string }[]
    ...
    // 开发环境模拟「连接打印」接口，GET /printers 返回本机系统打印机（Windows）
    {
      ...
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          ...
          if (!url.startsWith('/api/print/')) return next()
